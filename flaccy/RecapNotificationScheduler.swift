...
/// Schedules rich local Recap notifications from on-device play history. Content
/// is regenerated with fresh stats every time the app becomes active, so the
/// next pending notification always carries a current teaser and story-card
/// attachment. The annual Year in Music drop is scheduled alongside whichever
/// periodic cadence the user picks.
final class RecapNotificationScheduler {
    ...
    private static let testIdentifier = "recap.test"
    ...
    /// 1 = Sunday … 7 = Saturday, matching `Calendar` weekday numbering.
    ...
    func nextYearlyDeliveryDate() -> Date? {
    ...
    /// The exact title/body the next periodic notification would carry — used
    /// by the settings page to show a live preview.
    func previewContent() -> (title: String, body: String) {
        periodicPieces(frequency: frequency == .off ? .weekly : frequency)
    }

    /// Fires the real periodic notification a few seconds from now so the user
    /// can see exactly what they signed up for.
    func sendTestNotification() async -> Bool {
        ...
        let pieces = previewContent()
        content.title = pieces.title
        content.body = pieces.body
        ...
        do {
            ...
            AppLogger.info("Test recap notification scheduled", category: .general)
            return true
        } catch {
            AppLogger.error("Test notification failed: \(error.localizedDescription)", category: .general)
            return false
        }
    }

    /// Persists the chosen cadence, requesting notification permission when
    /// turning on. Returns false when the user has permission denied.
    ...
    /// Rebuilds the pending notifications with up-to-date stats. Called on every
    /// app activation, but debounced: as long as fresh-enough requests are
    /// pending, foregrounding costs a single settings check and nothing else.
    ...
    private func schedulePeriodic(frequency: RecapNotificationFrequency) async {
        ...
        let content = UNMutableNotificationContent()
        let pieces = periodicPieces(frequency: frequency)
        content.title = pieces.title
        content.body = pieces.body
        content.sound = .default
        content.userInfo = [Self.destinationUserInfoKey: Self.yearInMusicDestination]
    ...
    private func makeStoryAttachment() async -> UNNotificationAttachment? {
        ...
        let image = YearInMusicStoryView.makeImage(slide: .overview, data: data, artwork: artwork, theme: theme, scale: 2)
